...
try:
    ...
    logger.info("winrt loaded — SMTC detection enabled")
except ImportError as e:
    WINRT_AVAILABLE = False
    logger.warning(
        f"winrt not available ({e}) — SMTC detection disabled. "
        "Install with: pip install winrt-Windows.Media winrt-Windows.Media.Control"
    )
...
async def get_smtc_track(ignored_apps: list[str] | None = None) -> dict | None:
    ...
    or None if nothing is playing / winrt is unavailable.
