...
- Skips ACRCloud fingerprinting entirely (so it doesn't rack up API costs) when the API reports that no stream is currently live, and also skips posting now-playing data in that case.
...
| `app/smtc.py` | Windows Media Session (SMTC) detection via pywinrt |
...
Euterpium includes a Playnite generic plugin that automatically tells Euterpium when a game starts and stops. When a game is launched via Playnite, the plugin POSTs to Euterpium's local REST API (`/api/game/start`); Euterpium switches to game audio fingerprinting mode immediately. Works for Steam, GOG, Epic, emulators — any source Playnite can launch.
