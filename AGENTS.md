...
app/                Python application source + tests
  ...
  smtc.py           Windows SMTC detection (winrt, Windows-only)
...
Non-Windows: `winrt-*` / `win11toast` are platform-conditional deps. SMTC tests are
...
- Tries `from winrt.windows.media import ...` at module level.
- On failure (non-Windows, winrt not installed) sets `WINRT_AVAILABLE = False`
  and does **not** define `MediaManager` / `MediaPlaybackStatus` names. Code
  that references those names must be gated on `WINRT_AVAILABLE`.
